...
const formSchema = (dictionary: any) => z.object({
    date: z.date({
        required_error: dictionary.validation.date,
    }),
    factorId: z.string().min(1, { message: dictionary.validation.material }),
    quantity: z.coerce.number().min(0.01, { message: dictionary.validation.quantity }),
});
...
export function ValorizedResidueDialog({ open, onOpenChange, dictionary, emissionFactors, onResidueAdded }: ValorizedResidueDialogProps) {
  const { user } = useAuth();
  ...
  const onSubmit = async (values: FormSchema) => {
    ...
    try {
        const newResidueData = {
            ...
            unit: selectedFactor.unit.split('/')[1] || 'unit', // e.g., kg from kg CO2e/kg
            date: values.date,
                ...
                <div className="grid gap-4 py-4">
                              ...
                              <PopoverTrigger asChild>
                                <FormControl>
                                  <Button variant={"outline"} className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>
                                    {field.value ? format(field.value, "PPP") : <span>{dictionary.date.placeholder}</span>}
                          ...
                          <FormItem>
                            <FormLabel>{dictionary.material.label}</FormLabel>
                             <Select onValueChange={field.onChange} defaultValue={field.value}>
                              <FormControl>
                                <SelectTrigger>
                                  <SelectValue placeholder={dictionary.material.placeholder} />
                                </SelectTrigger>
                              </FormControl>
                              <SelectContent>
                                {emissionFactors
                                .filter(f => f.category === 'Residuos' || f.category === 'Materiales')
                                .map((factor) => (
                     ...
                     <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input type="number" step="0.01" placeholder={dictionary.quantity.placeholder} {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    {calculatedEmissions > 0 && (
                        <div className="rounded-lg border bg-emerald-50 border-emerald-200 p-4 text-center">
                            <p className="text-sm font-medium text-emerald-800">{dictionary.emissionsResult}</p>
                            <p className="text-2xl font-bold text-emerald-600">{calculatedEmissions.toFixed(2)} kg CO2e</p>
                        </div>
                    )}
                </div>
                <DialogFooter>
                    <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} disabled={isLoading}>{dictionary.cancel}</Button>
